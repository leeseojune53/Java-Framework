...
public class TransactionAOP {
    ...
    public static Map<Class, Object> makeTransactionProxyClass(Map<Class, Object> proxyServices) {
        ...
        for (Method method : methods) {
            transactionProxyClassMap
                    .computeIfAbsent(method.getDeclaringClass(), k -> new ArrayList<>())
                    .add(method);
        }
        ...
        return Enhancer.create(clazz, (MethodInterceptor) (obj, method, args, proxy) -> {
            if (methods.contains(method)) {
                ...
                try {
                    transaction.begin();
                    System.out.println(
                            "Class: " + clazz.getName() + "  Method : " + method.getName() + "CGLIB Transaction Begin");
                    transaction.getConnection().select("SELECT id FROM tbl_weekend_meal", Object.class);
                    Object result = proxy.invokeSuper(obj, args);
                    transaction.commit();
                    System.out.println("Class: " + clazz.getName() + "  Method : " + method.getName()
            ...
            System.out.println("Class: " + clazz.getName() + "  Method : " + method.getName()
                    + "Not Transactional Annotated Method");
